...
// We configure this to use the real SMTP if env vars are provided,
// otherwise fall back to Ethereal for local development testing.
...
async function initMailer(): Promise<void> {
  if (process.env.SMTP_HOST && process.env.SMTP_USER) {
    ...
    // Fallback to Ethereal Email for local dev
 ...
 */
export const sendMfaEmail = async (to: string, code: string): Promise<nodemailer.SentMessageInfo> => {
